...
function loadMould() {
    ...
    var f1 = 0; //开往新开河
    var f2 = 0; //开往上海体育馆
    var url = "ajax/Handler.php?Method=station&roadline=" + RoadLine;
    $.ajax({
        url: url,
        type: 'GET',
        dataType: 'text',
...
function TogetAll() {
    ...
    var html1 = "";
    var html2 = "";
    var url = "ajax/Handler.php?Method=gpsdata&roadline=" + RoadLine;
...
//                html1 += '                 <tr><td>广告品牌</td><td><a href="' + adurl + '">' + jdata[i].AdContent + '</a></td></tr>';
                //html1 += '                 <tr><td>司机姓名</td><td>' + jdata[i].drivername + '</td></tr>';
            ...
            if (jdata.length > 0) {
                for (var i = 0; i < jdata.length; i++) {
                    ...
                    if (jdata[i].state == "营运车辆") {
                        ...
                        $("#d" + jdata[i].vnumber + " table").find("tr:eq(5)").find("td:eq(1)").text(jdata[i].DWTime);
...
//                            lastup += '                 <tr><td>广告品牌</td><td><a href="' + adurl + '">' + jdata[i].AdContent + '</a></td></tr>';
                            //lastup += '                 <tr><td>司机姓名</td><td>' + jdata[i].drivername + '</td></tr>';
...
//                            lastdown += '                 <tr><td>广告品牌</td><td><a href="' + adurl + '">' + jdata[i].AdContent + '</a></td></tr>';
                            //lastdown += '                 <tr><td>司机姓名</td><td>' + jdata[i].drivername + '</td></tr>';
...
function getScreen() {
    var url = "ajax/Handler.php?roadline=" + RoadLine + "&Method=departscreen&startstation=all";
    $.ajax({
        url: url,
        type: 'GET',
        dataType: 'text',
...
function getStartScreen() {
    ...
    var html1 = "";
    var html2 = "";
    var url = "interface/Handler.php.ashx?action=departscreen&userid=test&password=test&roadline=" + RoadLine + "&startstation=all&format=xml";
    $.get(url, function (data) {
...
function getWorkBus() {
    ...
    var url = "Ajax/Handler.php.ashx?Method=workBus&roadline=" + RoadLine;
    $.get(url, function (data) {
        $("#WorkCount").html(data);
    });
}
